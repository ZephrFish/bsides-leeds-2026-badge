...
// Blinks "ZEPHRFISH" in Morse code on both eyes (red).
// Each byte is one Morse element: low nibble = ON duration in units
// (1 = dot, 3 = dash), high nibble = the gap that follows in units
// (1 = between elements, 3 = between letters, 7 = end of word).
static const uint8_t MORSE_UNIT_MS = 120;
static const uint8_t MORSE_LENGTH = 29;
static const uint8_t MORSE_TOTAL_UNITS = 92;
const uint8_t ZEPHRFISH_MORSE[] PROGMEM = {
  0x13, 0x13, 0x11, 0x31,        // Z  -- ..
  0x31,                          // E  .
  0x11, 0x13, 0x13, 0x31,        // P  .--.
  0x11, 0x11, 0x11, 0x31,        // H  ....
  0x11, 0x13, 0x31,              // R  .-.
  0x11, 0x11, 0x13, 0x31,        // F  ..-.
  0x11, 0x31,                    // I  ..
  0x11, 0x11, 0x31,              // S  ...
  0x11, 0x11, 0x11, 0x71,        // H  ....
};

uint8_t morseMode(uint16_t step)
{
  step = step % MORSE_TOTAL_UNITS;

  uint16_t cursor = 0;
  uint8_t litUnits = 0;  // 0 = off, 1 = dot (cyan), 3 = dash (purple)

  for (uint8_t elementIndex = 0; elementIndex < MORSE_LENGTH; ++elementIndex) {
    const uint8_t element = pgm_read_byte(&ZEPHRFISH_MORSE[elementIndex]);
    const uint8_t onUnits = element & 0x0F;
    const uint8_t gapUnits = element >> 4;

    if (step < cursor + onUnits) {
      litUnits = onUnits;
      break;
    }
    cursor += onUnits;

    if (step < cursor + gapUnits) {
      break;
    }
    cursor += gapUnits;
  }
  ...
  if (litUnits == 1) {
    setAllLeds(0, 30, 30, true);    // dot  -> cyan
  } else if (litUnits == 3) {
    setAllLeds(20, 0, 30, true);    // dash -> purple
  } else {
    setAllLeds(0, 0, 0, true);      // gap  -> off
  }

  return MORSE_UNIT_MS;
}

// Pride flag rainbow: six stripes spread across the LEDs, rotating over time.
// Bytes are stored in the strip's native GRB order and copied straight into
// the pixel buffer (cheaper than 18 setPixelColor calls on a near-full flash).
const uint8_t PRIDE_GRB[] PROGMEM = {
   0, 30,  0,   // red
  12, 30,  0,   // orange
  24, 24,  0,   // yellow
  30,  0,  0,   // green
...
uint8_t prideMode(uint16_t step)
{
  uint8_t src = (step % 6) * 3;

  for (uint8_t i = 0; i < NUM_LEDS * 3; ++i) {
    pixelBuffer[i] = pgm_read_byte(&PRIDE_GRB[src]);
...
// Each eye split into a red half and a blue half, purple where they meet in
// the middle (red + blue). Same pattern on both eyes.
uint8_t splitMode(uint16_t step)
{
  (void)step;

  for (uint8_t eye = 0; eye < 2; ++eye) {
    const uint8_t base = eye * LEDS_PER_EYE;

    for (uint8_t i = 0; i < LEDS_PER_EYE; ++i) {
      if (i < 4) {
        ledStrip.setPixelColor(base + i, 30, 0, 0);    // red half
      } else if (i == 4) {
        ledStrip.setPixelColor(base + i, 20, 0, 30);   // purple middle
      } else {
        ledStrip.setPixelColor(base + i, 0, 0, 30);    // blue half
      }
    }
  }
...
int runAnimationMode(uint8_t mode, uint16_t step)
{
  switch (mode) {
    case 0:
      return knightRider(step, 20, 0, 30);   // purple
    case 1:
      return breath(step, 0, 1, 1);          // cyan (breath multiplies by step 0..30)
    case 2:
      return loopingEyes(step, 30, 12, 0);   // orange
    case 3:
      return 0;  // freed: was knight-rider (red), a recolor duplicate of mode 0
    case 4:
      return 0;  // freed: was looping-eyes (red), a recolor duplicate of mode 2
    case 5:
    ...
    case 9:
      return 0;  // freed: spinMode removed
    case 10:
      return timer(step);
    case 11:
      return morseMode(step);
    case 12:
      return prideMode(step);
    case 13:
      return splitMode(step);
    default:
      return -1;
  }
}
